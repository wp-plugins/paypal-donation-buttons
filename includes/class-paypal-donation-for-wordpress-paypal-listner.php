 ...
 */
class Paypal_Donation_For_WordPress_PayPal_listner {
    ...
    public function check_ipn_request_is_valid($ipn_response) {
        ...
        $params = array(
            ...
            'compress' => false,
            'decompress' => false,
            'user-agent' => 'paypal-donation-buttons/'
        );
    ...
    public function ipn_response_data_handler($posted = null) {
        ...
        if (isset($posted) && !empty($posted)) {
            ...
            if ($this->paypal_donation_for_wordpress_exist_post_by_title($paypal_txn_id) == false) {
                ...
                $post_id = $this->paypal_donation_for_wordpress_exist_post_by_title($paypal_txn_id);
    ...
    /**
     * paypal_donation_for_wordpress_exist_post_by_title helper function used for check txn_id as post_title is exist or not
     * @since    1.0.0
     * @access   public
     */
    function paypal_donation_for_wordpress_exist_post_by_title($ipn_txn_id) {

        global $wpdb;

        $post_data = $wpdb->get_col($wpdb->prepare("SELECT ID FROM wp_posts WHERE post_title = %s AND post_type = %s ", $ipn_txn_id, 'donation_list'));
